...
class Fahrer extends Page
{
    ...
    protected function generateView():void
    {
        ...
        $this->generatePageHeader('Fahrer', 'pizzaScript.js'); //to do: set optional parameters
        echo "<body>";
        if($data){
            $numberOrders = 1;
...
EOT;
                for($j=$index; $j<sizeof($data)-1; ++$j){
                    ...
                    echo htmlspecialchars($data[$j]["name"]);
                    echo ", ";
                    if(!($data[$j]["ordering_id"] == $data[$j+1]["ordering_id"])){
                        //letzte zeile ausgeben
                        $ordering_id = $data[$j]["ordering_id"];
                        echo <<<EOT
                            <br><h3>Summe: $priceTotal €</h3>
                            <form action="fahrer.php" method="post" id="$ordering_id">
                                <br><label><input type="radio" name="status" value="2" onclick="submitOnClick($ordering_id)" $fertig>fertig</label>
                                <br><label><input type="radio" name="status" value="3" onclick="submitOnClick($ordering_id)" $unterwegs>unterwegs</label>
                                <br><label><input type="radio" name="status" value="4" onclick="submitOnClick($ordering_id)" $geliefert>ausgeliefert</label>
...
EOT;
                ...
                if(($index == (sizeof($data)-1)) && $i == $numberOrders-1){
                    $fertig = "";
                    $unterwegs = "";
                    $geliefert = "";
                    if($data[$index]["status"] == 2){
                    ...
                    $bestellNr = $data[$index]["ordering_id"];
                    $bestellNr = htmlspecialchars($bestellNr);
                    $priceTotal += (double)$data[$index]["price"];
                    $ordering_id = $data[$index]["ordering_id"];
                    echo <<<EOT
                        $namePizza
                        <br><h3>Summe: $priceTotal €</h3>
                        <form action="fahrer.php" method="post" id="$ordering_id">
...
EOT;
            ...
            if($index < (sizeof($data) - 1)){
                ...
                $ordering_id = $data[$index+1]["ordering_id"];
                echo <<<EOT
                    $namePizza
                    <br><h3>Summe: $priceTotal €</h3>
                    <form action="fahrer.php" method="post" id="$ordering_id">
